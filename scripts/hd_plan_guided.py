...
def load_model(args, height, ll=False):
...
hl_policies = []
for height in range(1, hlargs.maximal_height):
    _, hl_policy, hl_diffusion_experiment, hl_value_experiment, hl_logger = load_model(
        hlargs, height
    )
    hl_policies.append(hl_policy)
dataset, ll_policy, ll_diffusion_experiment, ll_value_experiment, ll_logger = (
    load_model(llargs, height, ll=True)
)

#### hlguidedpolicy
policy = HlGuidedPolicy(hl_policies=hl_policies, ll_policy=ll_policy, jump=hlargs.jump)
