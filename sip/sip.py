...
class PartitionTree():
    ...
    def __build_k_tree(self,g_vol,nodes_dict:dict,k = None,):
        ...
        while unmerged_count > 1:
            ...
            merge(new_id, id1, id2, cut_v, nodes_dict, self.adj_matrix)
    ...
    def path_entropy(self, id, ent):
        ...
        node_vol = node.vol
        node_g = abs(node.g)
        node_p_vol = self.tree_node[pid].vol
        if node_vol != 0 or node_p_vol != 0:
            ent += - (node_g / self.VOL) * math.log2(node_vol / node_p_vol)
        else:
            ent += 0.0
    ...
    def community_entropy(self, leaves):
        nodes = set()
        for lid in leaves:
            node = self.tree_node[lid]
            while node.parent is not None:
                nodes.add(node.ID)
                node = self.tree_node[node.parent]
        ent = 0.0
        for nid in nodes:
            node = self.tree_node[nid]
            node_vol = node.vol
            node_g = node.g
            node_p_vol = self.tree_node[node.parent].vol
            ent += - (node_g / self.VOL) * math.log2(node_vol / node_p_vol)
        return ent
